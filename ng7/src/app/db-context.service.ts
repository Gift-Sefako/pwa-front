...
export class DbContextService {
  dbName = 'pwa-indexed-db';
  dbTableName = 'failedRequests';
  dbVersion = 1;
  db = new NgxIndexedDB(this.dbName, this.dbVersion);

  constructor() {
     this.db.openDatabase(this.dbVersion, upgradeDb => {
...
;
    this.db.add( 'pwa-indexed-db', {request: obj}).then(() => {
       console.log('added request to indexed db ');
     }, error => {
       console.log(error);
     });
    /*
